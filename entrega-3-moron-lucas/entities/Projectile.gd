...
func _on_TurretProjectileArea_body_entered(body):
	if body is Player:
		body.get_hit()

func _on_PlayerProjectileArea_body_entered(body):
	if body is Turret:
		body.get_hit()


func _on_PlayerProjectileArea_area_entered(area):
	if area.name == "TurretBodyArea2D":
